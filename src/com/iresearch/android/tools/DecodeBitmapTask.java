...
public class DecodeBitmapTask extends AsyncTask<Void, Void, Bitmap> {
     ...
     * @param path
     */
    public DecodeBitmapTask(final int maxWidth, final int maxHeight, final String path) {
    ...
    protected Bitmap decode() {
        ...
        if (options.outWidth > maxWidth || options.outHeight > maxHeight)
            scale = Math.max(Math.round((float) options.outHeight / (float) maxHeight),
        ...
        if (Log.isLoggable(TAG, DEBUG)) {
            if (decoded == null) {
                ...
                Log.d(TAG, "Decoded to " + decoded.getWidth() + "x" + decoded.getHeight()
                        + " from max size: " + maxWidth + "x" + maxHeight + " using scale:" + scale
                        + " and byte count:" + getByteCount(decoded));
        }
