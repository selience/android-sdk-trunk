 ...
 * @link https://github.com/jesperborgstrup/buzzingandroid/blob/master/src/com/buzzingandroid/content/QueryBuilder.java
 */
public class QueryBuilder {
     ...
     * @param extraSelectionArgs One argument for every question mark in the
     *            selection string
     * @return The QueryBuilder instance for chaining method calls
     */
    ...
    public Cursor query(Context context, Uri uri) {
        ...
        return context.getContentResolver().query(uri, projection, builtSelection.first,
                builtSelection.second, sortOrder);
    }
     ...
     * @param uri
     * @return
     * @throws RemoteException May be thrown from
    ...
    public Cursor query(ContentProviderClient provider, Uri uri) throws RemoteException {
        validateForQuery();
        Pair<String, String[]> builtSelection = buildSelection();
        return provider.query(uri, projection, builtSelection.first, builtSelection.second,
                sortOrder);
    }
    ...
    public void queryAsync(Context context, Uri uri, final AsyncQueryCallback callback) {
        ...
        Pair<String, String[]> builtSelection = buildSelection();
        aqh.startQuery(0, null, uri, projection, builtSelection.first, builtSelection.second,
                sortOrder);
    }
    ...
    public CursorLoader createCursorLoader(Context context, Uri uri) {
        validateForQuery();
        Pair<String, String[]> builtSelection = buildSelection();
        return new CursorLoader(context, uri, projection, builtSelection.first,
                builtSelection.second, sortOrder);
    }
    ...
    public int update(Context context, ContentValues values, Uri uri) {
        Pair<String, String[]> builtSelection = buildSelection();
        return context.getContentResolver().update(uri, values, builtSelection.first,
     ...
     */
    public int update(ContentProviderClient provider, ContentValues values, Uri uri)
    ...
    public ContentProviderOperation createUpdateOperation(ContentValues values, Uri uri) {
        ...
        return ContentProviderOperation.newUpdate(uri)
                .withSelection(builtSelection.first, builtSelection.second).withValues(values)
                .build();
    }
    ...
    public int delete(Context context, Uri uri) {
        ...
        return context.getContentResolver()
                .delete(uri, builtSelection.first, builtSelection.second);
    }
    ...
    public ContentProviderOperation createDeleteOperation(Uri uri) {
        ...
        return ContentProviderOperation.newDelete(uri)
                .withSelection(builtSelection.first, builtSelection.second).build();
    }
